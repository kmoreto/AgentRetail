sap.ui.define([
  ...
  Storage, TablePersoController, Dialog, Button, List, CustomListItem, Input, HBox, VBox, Text,
  History, Fragment, jQuery, SuggestUtil, SupplierAlertsUtil
) {
  "use strict";
 ...
 var PO_LINK_BASE = "http://localhost:8082/test/flp.html?sap-ui-xx-viewCache=false#app-preview&/detail/4500515037";
  ...
  return Controller.extend("agentretail.controller.View2", {
    ...
    var oJoule = new sap.ui.model.json.JSONModel({
        visible: true,
        messages: [
            { from: "bot", text: "Olá, eu sou a Joule (mock). Como posso ajudar?" }
        ]
    });
    this.getView().setModel(oJoule, "joule");
},

    // ===== Joule (mock) panel handlers =====
    // Toggle visibility of the inline mock Joule panel (bound to `joule>/visible`)
    onToggleJoule: function () {
      // Instead of toggling the inline panel, open a floating dialog styled as a chat
      if (!this._floatingJoule) {
        this._createFloatingJoule();
      }
      if (this._floatingJoule.isOpen && this._floatingJoule.isOpen()) {
        this._floatingJoule.close();
      } else {
        this._floatingJoule.open();
        // focus input inside dialog
        var that = this;
        setTimeout(function () {
          var oInp = that._floatingJoule && that._floatingJoule.getContent && that._floatingJoule.getContent()[0] && that._floatingJoule.getContent()[0].byId && that._floatingJoule.getContent()[0].byId("jouleInput");
          if (oInp && oInp.focus) { try { oInp.focus(); } catch (e) {} }
        }, 250);
      }
    },

    // Handle sending a message from the mock Joule input. Appends a user message then a mock bot reply.
    onSendJoule: function (oEvent) {
      // prefer dialog input if floating dialog exists
      var oInput = null;
      if (this._floatingJoule) {
        oInput = sap.ui.getCore().byId(this.getView().getId() + "--jouleInputDlg");
      }
      if (!oInput) { oInput = this.byId("jouleInput"); }
      if (!oInput) { return; }
      var sText = (oInput.getValue() || "").trim();
      if (!sText) { return; }

      var oJoule = this.getView().getModel("joule");
      if (!oJoule) { return; }

      var aMsgs = oJoule.getProperty("/messages") || [];
      aMsgs.push({ from: "user", text: sText });
      oJoule.setProperty("/messages", aMsgs);
      oInput.setValue("");

      // scroll list to bottom if present
      setTimeout(function () {
        try {
          var $list = jQuery("#" + sap.ui.core.Fragment && sap.ui.core.Fragment.get && sap.ui.core.Fragment.getId ? (sap.ui.core.Fragment.getId()) : "") ;
        } catch (e) { }
        try {
          var $ul = jQuery("#" + that.getView().getId() + "--jouleListDlg-list");
          if ($ul && $ul.length) { $ul.scrollTop($ul[0].scrollHeight); }
        } catch (e) {}
      }, 50);

      // mock assistant reply (simple echo / canned behavior)
      var that = this;
      setTimeout(function () {
        var msgs = oJoule.getProperty("/messages") || [];
        msgs.push({ from: "bot", text: "Joule (mock): recebi — '" + sText + "'" });
        oJoule.setProperty("/messages", msgs);
        // scroll after bot reply
        try {
          var $ul2 = jQuery("#" + that.getView().getId() + "--jouleListDlg-list");
          if ($ul2 && $ul2.length) { $ul2.scrollTop($ul2[0].scrollHeight); }
        } catch (e) {}
      }, 400);
    },

    // Create a floating non-modal dialog that resembles a chat panel and binds to the `joule` model
    _createFloatingJoule: function () {
      var that = this;
      // If already created, return
      if (this._floatingJoule) { return; }

      // Build dialog content using a small XML-like structure via JS controls
      var oList = new List({
        id: this.getView().getId() + "--jouleListDlg",
        items: {
          path: "joule>/messages",
          template: new CustomListItem({
            content: new VBox({
              items: [
                new Text({ text: "{= ${joule>from} === 'bot' ? 'Joule' : 'Você'}", class: "mockJouleFrom" }),
                new Text({ text: "{joule>text}", class: "mockJouleText" })
              ]
            })
          })
        }
      ...
      var oInput = new Input(this.getView().getId() + "--jouleInputDlg", { width: "100%", placeholder: "Message Joule...", submit: function () { that.onSendJoule(); } });

      var oFooter = new HBox({ justifyContent: "SpaceBetween", items: [ oInput, new Button({ text: "Enviar", press: function () { that.onSendJoule(); } }) ] });

      var oDlg = new Dialog({
        title: "Joule (Assistente)",
        content: [ oList, oFooter ],
        stretch: false,
        modal: false,
        draggable: true,
        resizable: false,
        state: "None",
        beginButton: new Button({ text: "Fechar", press: function () { oDlg.close(); } }),
        customHeader: null,
        contentWidth: "420px",
        contentHeight: "560px",
  ...
  // style as floating bottom-right chat
  oDlg.addStyleClass("floatingJouleDialog");

      // attach footer after creation (already added in constructor for safety)
      oDlg.setEndButton(new Button({ text: "", visible: false }));
      oDlg.attachAfterOpen(function () {
        // bind models from view so list reflects messages
        oDlg.setModel(that.getView().getModel("joule"), "joule");
      });

      this.getView().addDependent(oDlg);
      this._floatingJoule = oDlg;
    },

    // Ensure fixed-position mock panel is visible when app runs inside FLP or other containers
    onAfterRendering: function () {
      // keep existing viz frame config behavior if present
      if (this._configVizFrames) { try { this._configVizFrames(); } catch (e) {} }

      try {
        var oPanel = this.byId("mockJoule");
        if (oPanel && oPanel.getDomRef) {
          var dom = oPanel.getDomRef();
          if (dom && dom.parentNode && dom.parentNode !== document.body) {
        ...
        console && console.warn && console.warn("mockJoule move to body failed:", e);
...
_runGenerateOrderSequence: function () {
  ...
  return new Promise(function (resolve) {
    ...
    onCloseSuggestProgress: function(){ 
      if (this._dlgSuggest){ this._dlgSuggest.close(); } 
